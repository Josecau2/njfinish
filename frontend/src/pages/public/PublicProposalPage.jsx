...
export default function PublicProposalPage() {
  ...
  if (!proposal) {
    return (
      <div className="container py-5">
  <CAlert color="warning">{t('publicQuote.notFound')}</CAlert>
      </div>
    );
  }
        ...
        <CCardHeader>
          <div className="d-flex justify-content-between align-items-center">
            <div>
              ...
              {proposal?.customer?.name && <small className="text-muted">{t('publicQuote.forCustomer', { name: proposal.customer.name })}</small>}
            </div>
        ...
        <CCardBody>
          ...
          <div className="mt-4 d-flex gap-2">
            <CButton color="success" disabled={isLocked || accepting} onClick={onAccept}>
              {accepting ? t('publicQuote.acceptButton.accepting') : (isLocked ? t('publicQuote.acceptButton.alreadyAccepted') : t('publicQuote.acceptButton.accept'))}
            </CButton>
          </div>
        </CCardBody>
      </CCard>
