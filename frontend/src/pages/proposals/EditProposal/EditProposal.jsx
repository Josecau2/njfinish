...
import { Box, FormControl, Input, Flex, Link, Badge, FormLabel, Modal, ModalHeader, ModalBody, ModalFooter, Alert, Container, Card, CardBody, Icon, Button } from '@chakra-ui/react'
...
import { Copy, File, List, MoreHorizontal, Edit, Trash, Trash2, Calendar, Printer, Mail, FileContract, CheckCircle } from 'lucide-react'
...
const EditProposal = ({
  ...
  const effectiveIsContractor =
    typeof isContractor === 'boolean' ? isContractor : !!isUserContractor
    ...
    try {
      ...
      if (response.payload?.success === true) {
        Swal.fire(
          ...
          t('proposals.toast.successSend', 'Quote saved successfully'),
          'success',
        )
        navigate('/quotes')
      } else {
        ...
        if (
          ...
          /(missing|required selections|is missing)/i.test(apiMsg)
        ) {
          await Swal.fire({
            icon: 'warning',
        ...
        if (/locked/i.test(apiMsg)) {
          ...
          Swal.fire(
            ...
            apiMsg || t('proposals.toast.errorGeneric'),
            'error',
          )
        }
      }
      ...
      const message =
        error?.response?.data?.message || error?.message || t('proposals.toast.errorGeneric')
  ...
  return (
    <Container
      fluid
      className="dashboard-container edit-proposal-page"
    ...
    >
          ...
          {({
            ...
            return (
              <FormControl onSubmit={handleSubmit} className="proposal-summary-form">
                ...
                <div className="card border-0 shadow-sm mb-4">
                  <div className="card-body p-4">
                    <Flex className="g-3">
                      {canAssignDesigner && (
                        <Box xs={12} sm={6} lg={4}>
                          ...
                          <CreatableSelect
                            ...
                            value={
                              designerOptions.find((opt) => opt.value === values.designer) || null
    </div>
    </div>
    </div>
    </div>
                      ...
                      <Box xs={12} md={6} lg={4}>
                        <div style={{ position: 'relative' }}>
                          <FormLabel htmlFor="designDate">{t('common.designDate')}</FormLabel>
                          <DatePicker
                            id="designDate"
                          ...
                          {!isUserContractor && (
                            <CDropdown onClick={(e) => e.stopPropagation()}>
                              ...
                              >
                                ...
                                >
                                  <Icon as={Edit} className="me-2" /> Edit
                                </CDropdownItem>
                                <CDropdownItem
                                ...
                                >
                                  <Icon as={Trash} className="me-2" /> Delete
                                </CDropdownItem>
                                <CDropdownItem
                                ...
                                >
                                  <Icon as={Copy} className="me-2" /> Duplicate
                                </CDropdownItem>
                              </CDropdownMenu>
                            </CDropdown>
                          )}
                ...
                <hr />
                <CTabs>
                  <Flex variant="tabs" className="quote-tabs border-0">
                    <Box>
                      <Link
                  ...
                  <div className="proposal-actions-grid">
                    {isViewOnly ? (
                      <div className="w-100" style={{ gridColumn: '1 / -1' }}>
                        <Alert status="success" className="text-center">
                          <h5 className="alert-heading mb-3">
                            <FaCheckCircle className="me-2" />
                            {t('proposals.lockedStatus.title')}
                          </h5>
                          <p className="mb-2">{t('proposals.lockedStatus.description')}</p>
      ...
      <Modal
        ...
        onClose={() =>
        <ModalOverlay>setEditModalOpen(false)}
        alignment="center"
        backdrop="static"
        className="modal-lg"
      >
        <ModalHeader closeButton>
          <ModalHeader>{t('common.editVersionName')}</ModalHeader>
        <ModalBody>
          <Input
            className="mb-3"
          ...
          <Button variant="outline" colorScheme="gray" onClick={() => setEditModalOpen(false)}>
            Cancel
          </Button>
      ...
      <Modal
        ...
        onClose={() =>
        <ModalOverlay>setDeleteModalOpen(false)}
        alignment="center"
        backdrop="static"
        className="modal-sm"
      >
        ...
        <ModalBody>{t('common.areYouSureDelete')}</ModalBody><ModalFooter>
          <Button variant="outline" colorScheme="gray" onClick={() => setDeleteModalOpen(false)}>
            Cancel
          </Button>
