  ...
  const sendToBackend = async (finalData, action = 'update') => {
    try {
      ...
      if (!finalData.id) {
        ...
        Swal.fire(t('common.error','Error'), t('proposals.errors.noId','Quote ID not found'), 'error');
        return;
      }
      ...
      if (action === 'accept' && finalData.manufacturerId && selectedVersion?.items) {
        ...
        if (!validation.isValid) {
          if (import.meta?.env?.DEV) console.warn('Sub-type validation failed in sendToBackend:', validation.missingRequirements);
