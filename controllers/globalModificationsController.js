...
function mapTemplateRow(r) {
	...
	const priceCents = r.price_cents != null ? Number(r.price_cents) : null;
	return {
		id: r.t_id ?? r.id,
		categoryId: r.category_id ?? r.c_id ?? null,
		name: r.t_name ?? r.name,
		priceCents,
		defaultPrice: priceCents != null ? (priceCents / 100) : null, // Convert cents to dollars
		fieldsConfig,
...
exports.createTemplate = async (req, res) => {
	try {
		let {
			...
			name,
			priceCents = null,
			defaultPrice = null, // Accept defaultPrice from frontend
			isReady = false,
			fieldsConfig = null,
			sampleImage = null,
			isBlueprint = false,
		...
		// Input normalization: accept camelCase or snake_case keys
		if (req.body?.category_id && !categoryId) categoryId = req.body.category_id;
		if (req.body?.price_cents != null && priceCents == null) priceCents = req.body.price_cents;
		if (req.body?.is_blueprint != null && isBlueprint == null) isBlueprint = !!Number(req.body.is_blueprint);
		if (req.body?.manufacturer_id != null && manufacturerId == null) manufacturerId = req.body.manufacturer_id;
		if (req.body?.fields_config && !fieldsConfig) fieldsConfig = req.body.fields_config;

		// Convert defaultPrice to priceCents if provided
		let finalPriceCents = priceCents;
		if (defaultPrice !== null && priceCents === null) {
			const parsed = parseFloat(defaultPrice);
			finalPriceCents = Number.isFinite(parsed) ? Math.round(parsed * 100) : null;
		}
		...
		// Enforce category scope/manufacturer compatibility if categoryId provided
		if (categoryId) {
			const [[cat]] = await sequelize.query(
				'SELECT id, scope, manufacturer_id FROM global_modification_categories WHERE id = ?',
				{ replacements: [Number(categoryId)] }
			);
			if (!cat) {
				return res.status(400).json({ message: 'Category not found' });
			}
			if (isBlueprint) {
				if (cat.scope !== 'gallery' || cat.manufacturer_id != null) {
					return res.status(400).json({ message: 'Blueprints must be assigned to gallery categories' });
				}
			} else {
				if (cat.scope !== 'manufacturer' || Number(cat.manufacturer_id) !== Number(manufacturerId)) {
					return res.status(400).json({ message: 'Manufacturer mods must use a category scoped to the same manufacturer' });
				}
			}
		}
		...
		const insertResult = await sequelize.query(
			...
			{ replacements: [
				...
				isBlueprintVal,
				manufacturerId || null,
				finalPriceCents != null ? Number(finalPriceCents) : null,
				fieldsJson,
				sampleImage || null,
				isReadyVal
			] }
...
exports.updateTemplate = async (req, res) => {
	try {
		...
		// Input normalization for alternate keys
		if (req.body?.category_id && !categoryId) categoryId = req.body.category_id;
		if (req.body?.price_cents != null && priceCents == null) priceCents = req.body.price_cents;
		if (req.body?.fields_config && !fieldsConfig) fieldsConfig = req.body.fields_config;
		...
		// Enforce category scope/manufacturer compatibility on update if categoryId provided
		if (categoryId) {
			const [[cat]] = await sequelize.query(
				'SELECT id, scope, manufacturer_id FROM global_modification_categories WHERE id = ?',
				{ replacements: [Number(categoryId)] }
			...
			if (existing.is_blueprint) {
				...
				if (cat.scope !== 'manufacturer' || Number(cat.manufacturer_id) !== Number(existing.manufacturer_id)) {
					return res.status(400).json({ message: 'Manufacturer mods must use a category scoped to the same manufacturer' });
				}
			}
		}
		...
		await sequelize.query(
			...
			{ replacements: [
				categoryId || null,
				String(name).trim(),
				existing.is_blueprint ? null : (finalPriceCents != null ? Number(finalPriceCents) : null),
				fieldsJson,
				sampleImage || null,
				isReadyVal,
				Number(id)
...
exports.getCategories = async (req, res) => {
	try {
		const { scope, manufacturerId = null, includeTemplates = false } = req.query;

		// REQUIRE scope parameter - no default to prevent bleeding
		if (!scope) {
			return res.status(400).json({ message: 'scope parameter is required (gallery or manufacturer)' });
		}

		if (scope !== 'gallery' && scope !== 'manufacturer') {
			return res.status(400).json({ message: 'scope must be either "gallery" or "manufacturer"' });
		}

		let whereClause = 'WHERE c.scope = ?';
		...
		if (scope === 'manufacturer') {
			...
			whereClause += ' AND c.manufacturer_id IS NULL';
		}

		if (includeTemplates === 'true' || includeTemplates === true) {
			// Include templates with LEFT JOIN
			const [rows] = await sequelize.query(`
				SELECT c.id AS c_id, c.name AS c_name, c.scope, c.manufacturer_id, c.order_index, c.image AS c_image, c.description, c.created_at, c.updated_at,
					   t.id AS t_id, t.category_id, t.name AS t_name, t.price_cents, t.fields_config, t.sample_image, t.is_ready, t.is_blueprint, t.manufacturer_id AS t_manufacturer_id, t.created_at AS t_created_at, t.updated_at AS t_updated_at
				FROM global_modification_categories c
			...
			// Original behavior - categories only
			const [rows] = await sequelize.query(`
				SELECT * FROM global_modification_categories c
				${whereClause}
...
exports.createCategory = async (req, res) => {
	try {
		...
		if (e.name === 'SequelizeUniqueConstraintError' || e.code === 'ER_DUP_ENTRY') {
			...
			const scope = req.body.scope || 'gallery';
			const manufacturerId = req.body.manufacturerId;

			let scopeMsg = scope === 'gallery' ? 'in the Gallery' : `for this manufacturer`;
			return res.status(409).json({
				message: `Category name "${String(categoryName).trim()}" already exists ${scopeMsg}. Please choose a different name.`,
				duplicate: true,
				scope,
				manufacturerId
			});
...
exports.updateCategory = async (req, res) => {
	try {
		...
		// Get existing category to check scope constraints
		const [[existing]] = await sequelize.query(
		...
		// Handle unique constraint violation during update
		if (e.name === 'SequelizeUniqueConstraintError' || e.code === 'ER_DUP_ENTRY') {
			const categoryName = req.body.name || 'Unknown';
			return res.status(409).json({
				message: `Category name "${String(categoryName).trim()}" already exists in this scope. Please choose a different name.`,
				duplicate: true
			});
		}
...
exports.deleteCategory = async (req, res) => {
	try {
		...
		console.log(`[DELETE CATEGORY] ID: ${id}, Mode: ${mode}`);
		...
		console.log(`[DELETE CATEGORY] Found category:`, category);
		...
		if (mode === 'only') {
			...
			console.log(`[DELETE CATEGORY] Templates count in category ${id}:`, count.cnt);

			if (count.cnt > 0) {
				console.log(`[DELETE CATEGORY] Cannot delete - category has ${count.cnt} templates`);
				return res.status(400).json({
					message: `Cannot delete category: contains ${count.cnt} modifications. Use mode=withMods or mode=move.`
				});
			}
...
// Merge categories: move all templates from source to target, then delete source
exports.mergeCategories = async (req, res) => {
	try {
		...
		if (fromId === toId) {
			return res.status(400).json({ message: 'Cannot merge category into itself' });
		}

		// Verify both categories exist and get their scopes
		const [[fromCategory]] = await sequelize.query(
			'SELECT id, name, scope, manufacturer_id FROM global_modification_categories WHERE id = ?',
			{ replacements: [Number(fromId)] }
		);

		const [[toCategory]] = await sequelize.query(
			'SELECT id, name, scope, manufacturer_id FROM global_modification_categories WHERE id = ?',
			{ replacements: [Number(toId)] }
		);
		...
		// CRITICAL: Verify categories have the same scope and manufacturer to prevent bleeding
		if (fromCategory.scope !== toCategory.scope) {
			return res.status(400).json({
				message: `Cannot merge categories across different scopes. Source is ${fromCategory.scope}, target is ${toCategory.scope}.`
			});
		}

		if (fromCategory.manufacturer_id !== toCategory.manufacturer_id) {
			const fromManu = fromCategory.manufacturer_id || 'Gallery';
			const toManu = toCategory.manufacturer_id || 'Gallery';
			return res.status(400).json({
				message: `Cannot merge categories across different manufacturers. Source belongs to ${fromManu}, target belongs to ${toManu}.`
			});
		}

		// Move all templates from source to target category
		await sequelize.query(
			'UPDATE global_modification_templates SET category_id = ? WHERE category_id = ?',
			{ replacements: [Number(toId), Number(fromId)] }
		);
		...
		return res.json({
			message: `Successfully merged "${fromCategory.name}" into "${toCategory.name}"`,
			mergedFrom: fromCategory,
			mergedInto: toCategory
		});
	} catch (e) {
		console.error('mergeCategories error:', e);
		return res.status(500).json({ message: 'Failed to merge categories' });
	}
};

// Reassign template to a different category
exports.reassignTemplateCategory = async (req, res) => {
	try {
		const { id } = req.params;
		...
		// Verify template exists and get its scope
		const [[template]] = await sequelize.query(
			'SELECT id, name, is_blueprint, manufacturer_id FROM global_modification_templates WHERE id = ?',
			{ replacements: [Number(id)] }
		);
		...
		// Verify target category exists and get its scope
		const [[category]] = await sequelize.query(
			'SELECT id, name, scope, manufacturer_id FROM global_modification_categories WHERE id = ?',
			{ replacements: [Number(category_id)] }
		);
		...
		// CRITICAL: Enforce scope compatibility to prevent bleeding
		if (template.is_blueprint) {
			if (category.scope !== 'gallery') {
				return res.status(400).json({
					message: 'Blueprint templates can only be assigned to gallery categories'
				});
			}
			if (category.manufacturer_id !== null) {
				return res.status(400).json({
					message: 'Blueprint templates cannot be assigned to manufacturer-scoped categories'
				});
			}
		} else {
			// Manufacturer mod
			if (category.scope !== 'manufacturer') {
				return res.status(400).json({
					message: 'Manufacturer modifications can only be assigned to manufacturer categories'
				});
			}
			if (Number(category.manufacturer_id) !== Number(template.manufacturer_id)) {
				return res.status(400).json({
					message: `Manufacturer modification can only be assigned to categories of the same manufacturer (template: ${template.manufacturer_id}, category: ${category.manufacturer_id})`
				});
			}
		}
		...
		await sequelize.query(
			'UPDATE global_modification_templates SET category_id = ? WHERE id = ?',
			{ replacements: [Number(category_id), Number(id)] }
		);

		return res.json({
			message: `Template "${template.name}" reassigned to category "${category.name}"`,
			template: { id: template.id, name: template.name },
			category: { id: category.id, name: category.name }
		});
	} catch (e) {
		console.error('reassignTemplateCategory error:', e);
		return res.status(500).json({ message: 'Failed to reassign template category' });
	}
};
